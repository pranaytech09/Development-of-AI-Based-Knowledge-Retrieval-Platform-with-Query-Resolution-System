"""Optional stdout logging for LangGraph node/tool execution."""
...
def _enabled() -> bool:
    return bool(get_settings().execution_logging_enabled)


def _color(text: str, color: str) -> str:
    if not get_settings().execution_log_use_color:
        return text
...
def _truncate(value: Any, max_chars: int | None = None) -> str:
    ...
    limit = max_chars or get_settings().execution_log_max_chars
    if len(text) <= limit:
        return text
...
def log_route(name: str, decision: Any, state: Any | None = None) -> None:
    payload: dict[str, Any] = {"decision": _truncate(decision)}
...
def logged_node(name: str, fn: Callable[..., Any]) -> Callable[..., Any]:
    """Wrap a graph node so start/end/error events are logged."""
